...
export class TelegramCensor implements INodeType {
	description: INodeTypeDescription = {
		...
		inputs: ['main'],
		outputs: '={{ $parameter["operation"] === "downloadMedia" ? ["main", "main"] : ["main"] }}',
		...
		properties: [
				...
				options: [
					{
						name: 'Auto Detect',
						value: 'auto',
						description: 'Infer from MIME type or URL extension',
					},
					{ name: 'Photo', value: 'photo' },
					{ name: 'Video', value: 'video' },
					{ name: 'Document', value: 'document' },
			...
			},
			{
				...
				displayOptions: { show: { operation: ['editMessage'] } },
				description:
					'If enabled and Replace Media URL is empty, removes media from the target message.',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const requestedGramJsLogLevel = (
			process.env.TELEGRAM_CENSOR_GRAMJS_LOG_LEVEL || ''
		).toLowerCase();
		const resolvedGramJsLogLevel =
			requestedGramJsLogLevel === 'debug'
				? LogLevel.DEBUG
				: requestedGramJsLogLevel === 'info'
					? LogLevel.INFO
					: requestedGramJsLogLevel === 'warn' || requestedGramJsLogLevel === 'warning'
						? LogLevel.WARN
						: requestedGramJsLogLevel === 'error'
							? LogLevel.ERROR
							: requestedGramJsLogLevel === 'none'
								? LogLevel.NONE
		...
		const isDownloadTimeoutError = (error: unknown): boolean => {
			...
			const code =
				typeof maybeRpcError.code === 'number' ? Math.abs(maybeRpcError.code) : undefined;
			const rawMessage =
      ...
      extractErrorMessage(error).toUpperCase().includes('MEDIA_INVALID');
		...
		const getMediaMetadataFromMessage = (
			...
			const fileNameFromAttributes = Array.isArray(doc?.attributes)
				? (doc.attributes.find(
						(attr: any) => typeof attr?.fileName === 'string' && attr.fileName.trim() !== '',
		...
		const assertHostIsSafe = async (hostName: string): Promise<void> => {
			...
			if (!isHostAllowed(normalizedHost)) {
				throw new Error(
					`Media URL host "${normalizedHost}" is not in TELEGRAM_CENSOR_MEDIA_URL_ALLOWLIST.`,
				);
			}
			...
			if (addresses.some(({ address }) => isPrivateIpAddress(address))) {
				throw new Error(
					`Blocked Media URL host "${hostName}" because it resolves to a private or local IP.`,
				);
			}
		};
		...
		const validateMediaUrl = async (rawUrl: string): Promise<URL> => {
			...
			if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
				throw new Error(
					`Unsupported Media URL protocol "${parsed.protocol}". Only http/https are allowed.`,
				);
			}
		...
		const downloadUrlToBuffer = async (
			url: string,
		...
		try {
			...
			for (let i = 0; i < items.length; i++) {
				...
				try {
					...
					hasDownloadMediaOperation = hasDownloadMediaOperation || operation === 'downloadMedia';
					...
					switch (operation) {
						...
						case 'getMessages': {
							...
							const maxMessages = toInt(
								this.getNodeParameter('maxMessages', i, 500),
								'Max Messages',
							);
							...
							if (mode === 'limit') {
								...
								const toTime = toDateStr
									? parseDateToUnix(toDateStr, 'To Date')
							...
							for (const msg of messages) {
								...
								const isVideo =
									!!msg.media?.video ||
									(isDocument && msg.media.document.mimeType?.includes('video'));
						...
						case 'sendMessage': {
							...
							const chatId = sendToSelf
								? 'me'
								: (this.getNodeParameter('sendChatId', i, '') as string);
							const text = this.getNodeParameter('sendText', i, '') as string;
							const parseMode = this.getNodeParameter('sendParseMode', i, 'markdownv2') as
								| 'markdownv2'
								| 'html';
							...
							if (attachMedia) {
								const selectedType = this.getNodeParameter('sendMediaType', i, 'auto') as
									| 'auto'
									| 'photo'
									| 'video'
									| 'document';
							...
							successData.push(
								makeLightweightItem(
									item.json,
									{
										...
										replyToId:
											(sentMessage as any)?.replyTo?.replyToMsgId ?? (replyTo > 0 ? replyTo : null),
									},
									i,
								),
							);
						...
						case 'downloadMedia': {
							...
							const messageId = toInt(
								this.getNodeParameter('downloadMessageId', i, 0),
								'Message ID',
							);
						...
						case 'nudeNetBlur': {
							if (!this.helpers.assertBinaryData(i, 'media')) {
								throw new Error(
									'Missing binary data property "media". Connect a Scanner node first.',
								);
							}
						...
						case 'editMessage': {
							...
							const editZeroMedia = this.getNodeParameter('editZeroMedia', i, false) as boolean;
							const editMediaUrl = this.getNodeParameter('editMediaUrl', i, '') as string;
							const hasReplaceMediaUrl = editMediaUrl.trim() !== '';
							const binaryEntries = Object.entries(item.binary ?? {}) as Array<[string, any]>;
							const preferredBinaryEntry =
								binaryEntries.find(([name]) => name === 'media') || binaryEntries[0];
							const inputBinaryProperty = preferredBinaryEntry?.[0];
							const inputBinaryData = preferredBinaryEntry?.[1];
							...
							if (hasReplaceMediaUrl) {
								const { buffer, fileName, mimeType } = await downloadUrlToBuffer(editMediaUrl.trim());
								const urlExtension = getExtensionForMime(mimeType);
								...
								for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
									try {
										...
										if (attempt < maxAttempts) {
											await sleepMs(zeroMediaRetryBackoffMs[attempt - 1]);
										}
									}
								}

								if (lastError) {
									// Telegram often rejects InputMediaEmpty with MEDIA_INVALID for media messages.
									// Fallback behavior: delete original message, then optionally send caption text as a new message.
									if (isMediaInvalidError(lastError)) {
										try {
											await client.deleteMessages(chatId, [messageId], { revoke: true });

											if (caption.trim() !== '') {
												await client.sendMessage(chatId, { message: caption });
												action = 'Original media message deleted and caption posted as new message';
											} else {
												action = 'Original media message deleted';
											}
										} catch (fallbackError) {
											const original = extractErrorMessage(lastError);
											const fallback = extractErrorMessage(fallbackError);
											throw new Error(
								...
								const buffer = await this.helpers.getBinaryDataBuffer(i, selectedBinaryProperty);
								const binaryExtension = getExtensionForMime(selectedBinaryData.mimeType);
							...
							const wasBlurred =
								item.json.blurred === true || Number(item.json.detectionCount ?? 0) > 0;
					...
					if (this.continueOnFail()) {
						...
						successData.push(makeLightweightItem(item.json, { error: errorMessage }, i));
						continue;
					}
